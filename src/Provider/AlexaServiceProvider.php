...
namespace ByronTudhope\AlexaApp\Provider;

use ByronTudhope\AlexaApp\Alexa;
use ByronTudhope\AlexaApp\Certificate\DatabaseCertificateProvider;
use ByronTudhope\AlexaApp\Certificate\EloquentCertificateProvider;
use ByronTudhope\AlexaApp\Certificate\FileCertificateProvider;
use ByronTudhope\AlexaApp\Certificate\RedisCertificateProvider;
use ByronTudhope\AlexaApp\Device\DatabaseDeviceProvider;
use ByronTudhope\AlexaApp\Device\EloquentDeviceProvider;
...
class AlexaServiceProvider extends ServiceProvider
{
    ...
    protected function registerRouter()
    {
        $this->app->singleton('alexa.router', function ($app) {
            return $app->make(\ByronTudhope\AlexaApp\Http\Routing\AlexaRouter::class);
        });
    }
    ...
    {
        $this->app->singleton(\ByronTudhope\AlexaApp\Http\Middleware\Request::class, function ($app) {
            return new \ByronTudhope\AlexaApp\Http\Middleware\Request($app, $app['alexa.router'], $app['alexa.request'], $app['alexa.router.middleware']);
        });

        $this->app->singleton(\ByronTudhope\AlexaApp\Http\Middleware\Certificate::class, function ($app) {
            return new \ByronTudhope\AlexaApp\Http\Middleware\Certificate($app['alexa.request'], $app['alexa.certificateProvider'], $app['config']['alexa']);
        });
    }
